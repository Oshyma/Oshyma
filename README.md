<div align="center">
<img src="https://github.com/Oshyma/Oshyma/blob/main/promethea.png" width="800" alt="Promethea Corp" />

# Hey, I'm Oshyma

Founder of **Promethea Corp** — I build tools, bots, and experiences that make communities thrive.  
Passionate about clean architecture, gamified systems, and turning ideas into real products.

[![Discord](https://img.shields.io/badge/Discord-Join%20Promethea-5865F2?style=for-the-badge&logo=discord&logoColor=white)](https://discord.gg/bF3REtSEq2)
[![Email](https://img.shields.io/badge/Email-contact.promethea@gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:contact.promethea@gmail.com)

</div>

---

## About Me

- Founder of **Promethea Corp**, focused on Discord tooling and community automation
- Building gamified developer experiences (see [AscendOs](#featured-projects))
- Specialized in Discord bots — moderation, leveling, delivery systems, affiliation programs
- Always learning — currently exploring AI assistant development, VSCode extension development and SaaS solutions
- I care about **user experience as much as the code behind it**

---

## Tech Stack

**Languages**

![Java](https://skillicons.dev/icons?i=java)
![JavaScript](https://skillicons.dev/icons?i=js)
![C#](https://skillicons.dev/icons?i=cs)
![Python](https://skillicons.dev/icons?i=py)
![C++](https://skillicons.dev/icons?i=cpp)
![C](https://skillicons.dev/icons?i=c)
![Haskell](https://skillicons.dev/icons?i=haskell)

**Frameworks & Libraries**

![Node.js](https://skillicons.dev/icons?i=nodejs)
![Nuxt.js](https://skillicons.dev/icons?i=nuxtjs)
![Vue.js](https://skillicons.dev/icons?i=vue)
![React](https://skillicons.dev/icons?i=react)
![Express](https://skillicons.dev/icons?i=express)

**Game Engines**

![Godot](https://skillicons.dev/icons?i=godot)
![Unity](https://skillicons.dev/icons?i=unity)

**Web & Tools**

![HTML](https://skillicons.dev/icons?i=html)
![CSS](https://skillicons.dev/icons?i=css)
![GitHub](https://skillicons.dev/icons?i=github)

---

## Featured Projects


### [MeetingSync](https://github.com/Oshyma/meetingsync)
A micro-SaaS for automatic meeting transcription and summarization — upload recordings and receive structured email summaries with key points, decisions, and action items. Built with scalable processing pipelines, authentication, payments, and quota-based usage management.
`Next.js 14` `TypeScript` `Supabase` `OpenAI Whisper` `GPT-4o` `Stripe` `Resend`

### [PrometheaBot](https://github.com/PrometheaCorp/PrometheaBot)
A full-featured Discord bot for community management — XP leveling, anti-bot captcha, music playback (YouTube / Spotify / SoundCloud), moderation, audit logging, and server setup utilities.
`Discord.js v14` `DisTube` `SQLite` `Node.js`

---

### [Promethea Delivery](https://github.com/PrometheaCorp/Promethea-Delivery)
A Discord bot that handles end-to-end order management — delivery tickets, a complete affiliate program with unique codes and PayPal tracking, HTML transcripts, and cross-server order verification.
`Discord.js v14` `quick.db` `SQLite` `Node.js`

---

### [AscendOs](https://github.com/Oshyma/AscendOs)
A VS Code extension that turns coding into an RPG. Earn XP as you write code, level up, complete missions, and collect loot — all from your editor's status bar.
`TypeScript` `VS Code API` `ESBuild`

---

## 📊 GitHub Stats

<div align="center">

[![GitHub Stats](https://github-stats-extended.vercel.app/api?username=Oshyma&show_icons=true&include_all_commits=true&theme=github_dark)](https://github-stats-extended.vercel.app/api?username=Oshyma&show_icons=true&include_all_commits=true&theme=github_dark)

</div>
<div align="center">
  
[![GitHub Stats](https://github-stats-extended.vercel.app/api/wakatime?username=Oshyma&layout=compact&langs_count=6&theme=github_dark)](https://wakatime.com/@Oshyma)
</div>

---
<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&weight=100&size=14&duration=3000&pause=1000&color=F7CE00&center=true&vCenter=true&width=435&lines=Building+things+that+matter%2C+one+commit+at+a+time.)](https://git.io/typing-svg)

</div>
